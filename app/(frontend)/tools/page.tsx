...
function HubCard({ tool, index }: { tool: ToolCard; index: number }) {
  const accent =
    tool.accent === "amber" ? "var(--accent-2)" : "var(--accent-cool)";
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
        <Reveal>
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-[var(--panel)] p-8 text-center sm:p-12">
            ...
            <Link
              href="/#contact"
              className="mt-7 inline-flex h-12 items-center justify-center rounded-full bg-[var(--accent)] px-7 text-sm font-medium text-[var(--accent-ink)] shadow-[0_12px_32px_-12px_rgba(255,140,0,0.45)] transition-[transform,background-color] duration-300 hover:-translate-y-0.5 hover:bg-[#FFA31A]"
            >
              Book a Call
            </Link>
          </div>
